...
1. **Секрет удален или переименован**  
...
2. **Запуск из форка или PR из форка**  
   В таких запусках GitHub **не передает** секреты репозитория (политика безопасности). Запускайте workflow из ветки `main` основного репозитория (push в `main` или **Actions → Run workflow** с веткой `main`).
